...
using System.Timers;
//using Injector.Utilities.Exceptions;
...
namespace Injector.Core.Services.Settings
{
  ...
  public class SettingsCache : IEnumerable<SettingsObjectWrapper>, IDisposable
  {
    ...
    public IEnumerator<SettingsObjectWrapper> GetEnumerator()
    {
      lock (_syncObj)
        if (_timer.Enabled)
          throw new Exception("The enumerator on the settings cache may only be requested when the settings cache keeps its objects");
        else
